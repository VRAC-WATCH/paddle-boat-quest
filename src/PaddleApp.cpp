...
void PaddleApp::init()
{
	...
	mBoats.push_back(new Boat());
	mBoats.push_back(new Boat());
	mBoats.back()->setT(0.15);
	
	for(size_t i = 0; i < mBoats.size(); i++)
		mModelGroup->addChild(mBoats[i]->getRoot());
...
void PaddleApp::deToggleButtons()
{
...
void PaddleApp::setHeadMatrix(osg::Matrixf mat)
{
	mHeadMatrix = mat*mNavigation->getInverseMatrix();
}

void PaddleApp::setWandMatrix(osg::Matrixf mat)
{
...
void PaddleApp::updateFrameRate(float dt)
{
...
void PaddleApp::handleArguments(int* argc, char **argv)
{
